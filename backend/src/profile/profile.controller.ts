...
export class ProfileController {
  ...
  @ApiOperation({
    summary: "Returns an image from the users' profile",
    description:
      "Sends an image from the users' profile located on uploads folder.",
  })
  public getImage(@Param('filename') filename: string, @Res() res) {
    if (
      filename === 'null' ||
      filename === 'undefined' ||
      !ProfileService.imageRegex.test(path.extname(filename))
    )
